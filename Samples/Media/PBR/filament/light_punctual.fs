...
void setupPunctualLight(inout Light light, const highp vec4 positionFalloff) {
    ...
    light.attenuation = mix(1.0, getDistanceAttenuation(posToLight, positionFalloff.w), positionFalloff.w);
...
Light getSpotLight(uint index) {
    ...
    highp vec4 positionFalloff = u_lightPositionFalloff[index];
    highp vec4 colorIntensity = u_lightColorIntensity[index];
    vec4 directionIES = u_lightDirection[index];
    vec2 scaleOffset;
    scaleOffset.x = 1.0 / max(u_lightSpotlightParams[index].x - u_lightSpotlightParams[index].y, 1e-4);
    scaleOffset.y = -u_lightSpotlightParams[index].y * scaleOffset.x;
    ...
    // We instead always use the spotlight function but do not change attenuation if it's not spotlight.
    // light.attenuation *= getAngleAttenuation(-directionIES.xyz, light.l, scaleOffset);
    light.attenuation *= mix(1.0, getAngleAttenuation(-directionIES.xyz, light.l, scaleOffset), u_lightSpotlightParams[index].w);
...
*/
    ...
    {
        // We also get directional light as point light and
        // it will work correctly with this algorithm so we don't
        // need the light_directional.fs
        Light light = getSpotLight(index);
#if defined(MATERIAL_CAN_SKIP_LIGHTING)
        if (light.NoL > 0.0) {
            color.rgb += surfaceShading(pixel, light, 1.0);
